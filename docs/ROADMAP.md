...
- [ ] 9.6 Contextualizar la entrevista con el puesto real (agregado 2026-08-11 — al planear la "sala de espera" de Frontend Fase 7, surgió que Gaby entrevista a todos igual, sin saber a qué puesto postuló el candidato ni sus requisitos; en Fase 9.5 se había dejado explícitamente afuera la conexión `Interview`↔`Postulacion` por no tener un uso concreto todavía — ahora sí lo hay).
  - [ ] 9.6.1 Campo `Interview.postulacion` (`ForeignKey` nullable a `apps.recruiting.models.Postulacion`, `on_delete=SET_NULL`) — nueva migración. Nota: esto crea una dependencia nueva de `apps.interviews` hacia `apps.recruiting` (antes no existía en ese sentido), aceptable porque es la relación real del dominio.
  - [ ] 9.6.2 En `apps/interviews/views.py` (`ask`), al crear una `Interview` nueva para un usuario autenticado: buscar su `Postulacion` con `estado="aprobado"` más reciente donde `email=request.user.email` (mismo criterio de matching por email que ya usa `apps/accounts/services/account_provisioning.py`) y conectarla en `postulacion=`.
  - [ ] 9.6.3 `ask_llm_task` arma un `system_prompt` contextual cuando la `Interview` tiene `postulacion` (título/descripción/requisitos del puesto), reusando el parámetro `system_prompt` opcional de `LLMProvider.ask()` (el mismo mecanismo agregado para el filtro de CVs en Fase 9.3, ver DECISIONS.md 2026-08-10) — sin entrevista/postulacion conectada (demo anónima), sigue usando el `INTERVIEW_SYSTEM_PROMPT` genérico de siempre, sin romper nada.
  - [ ] 9.6.4 Endpoint `GET /api/postulaciones/mia/` (autenticado): devuelve `nombre` + `puesto.titulo` (y lo que haga falta) de la `Postulacion` aprobada más reciente del usuario logueado — lo necesita la sala de espera de Frontend 7.2 para mostrar algo antes de que exista una `Interview` (que recién se crea con el primer mensaje).
...
- [ ] 7.1 Reordenar rutas: `/` pasa a ser `ApplyPage` (pública), la entrevista se mueve a `/entrevista` (protegida con `RequireAuth`, como hoy). Actualizar el link del logo en `Navbar` (`to="/postular"` → `to="/"`), y `LoginPage` (`navigate("/")` → `navigate("/entrevista")`, o a donde corresponda según 7.2).
- [ ] 7.2 Pantalla de "sala de espera" antes del chat en vivo — al entrar a `/entrevista`, mostrar primero una tarjeta de bienvenida **personalizada** ("¡Hola {nombre}! Vas a entrevistarte para el puesto de {puesto.titulo}. Contestá con texto o voz, tomate el tiempo que necesites"), usando el endpoint `GET /api/postulaciones/mia/` de Backend 9.6.4; con un botón "Empezar entrevista", recién al clickearlo se muestra el chat real (`QuestionDisplay`/`TextAnswerForm`/`VoiceRecorder`). Puede resolverse como un estado local nuevo en `InterviewPage` (`hasStarted`), sin necesitar una ruta aparte. Depende de Backend 9.6 (sin `Postulacion` conectada — demo anónima — mostrar el saludo genérico de antes, sin romper ese caso).
- [ ] 7.3 Bloquear la salida accidental de una entrevista activa sin terminar. Enfoque técnico: `useBlocker` de `react-router` (soportado por `createBrowserRouter`, que ya se usa) dentro de `InterviewPage`, activo mientras `hasStarted && !isFinished` — bloquea cualquier intento de navegación (no solo el logo, también el botón "atrás" del navegador) y pide confirmación antes de dejar salir. Más robusto que deshabilitar un solo link a mano, porque cubre todas las formas de navegar afuera, no una sola.
