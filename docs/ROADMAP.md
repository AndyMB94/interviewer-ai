...
- [ ] P.8 Pulido de interacción (agregado 2026-08-13, tras revisar el producto ya completo funcionalmente — confirmaciones nativas del navegador, loaders de puro texto, hover inconsistente). Alcance: toda la app, candidato y reclutador por igual.
  - [ ] P.8.1 Reemplazar `window.confirm` por `AlertDialog` de shadcn en los dos lugares donde se usa hoy: `InterviewPage.tsx` (bloqueo de salida de una entrevista activa, Frontend 7.3) e `InterviewDetailPage.tsx` (confirmar avanza/no avanza, Frontend 6.4). Requiere instalar el componente (`npx shadcn@latest add alert-dialog`) — no está instalado todavía. `DialogTitle`/`AlertDialogTitle` obligatorio por accesibilidad aunque el diseño no lo muestre visualmente grande.
  - [ ] P.8.2 `Skeleton` en vez de texto "Cargando..." — `PuestosPage.tsx`, `PostulacionesPage.tsx` (dashboard), `PuestoDetailPage.tsx`, `InterviewDetailPage.tsx`. Mismo patrón que `PuestoCardSkeleton` (Fase P.6.2), con la forma aproximada de cada pantalla (filas de tabla / secciones de card) en vez de un placeholder genérico.
  - [ ] P.8.3 `Spinner` de shadcn en botones con acción en curso — enviar postulación (`PuestoDetailPage`), avanza/no avanza (`InterviewDetailPage`). Hoy solo cambia el texto del botón ("Enviando...") o se deshabilita sin ningún indicador visual; según la regla del framework, un botón nunca tiene `isLoading` propio, se compone con `Spinner` + `disabled`. Requiere instalar el componente (`npx shadcn@latest add spinner`).
  - [ ] P.8.4 Hover/transición consistente — hoy solo `PuestoCard` tiene `hover:shadow-lg`; las filas de tabla del dashboard (`PuestosPage`, `PostulacionesPage`) y las cards de `PuestoDetailPage`/`InterviewDetailPage` no tienen ningún feedback visual al pasar el mouse. Agregar `transition-colors hover:bg-muted/50` (o similar, semántico — nunca colores crudos) a filas de tabla clickeables/informativas.
  - [ ] P.8.5 `LoginPage` — pulido adicional sobre lo ya hecho en P.6.3 (card centrada + ícono): transición de entrada sutil (fade/slide corto al montar) y asegurar que el botón de submit siga el mismo patrón de `Spinner` de P.8.3 mientras autentica, en vez de solo deshabilitarse.
  - [ ] P.8.6 Verificación visual en el navegador de cada punto — esta fase es puramente de UI, no lleva tests automáticos nuevos (nada de lógica de negocio cambia), pero sí una pasada manual confirmando que nada quedó roto (formularios, navegación, dark/light mode).
