...
- [ ] P.11 Título "Feedback final" es inconsistente con la decisión de producto ya tomada (encontrado 2026-08-13, probando P.10 en el navegador). `QuestionDisplay.tsx` línea 36 pone `{isFinished ? "Feedback final" : "Entrevista"}` como título de la card al terminar la entrevista — pero el contenido que se muestra ahí es solo el mensaje neutro "Gracias por su tiempo... Nos vamos a poner en contacto por email con los siguientes pasos", **sin ningún feedback real**. Esto contradice la decisión ya tomada en Fase P.4 (ver DECISIONS.md): el candidato nunca ve una evaluación de la IA, ni al finalizar la entrevista, igual que nunca ve el `resultado_filtro` del CV. El título "Feedback final" es un nombre que sobrevivió de antes de esa decisión — le promete al candidato algo que la pantalla no le da.
  - [ ] P.11.1 Cambiar el título a algo neutro, ej. `"Entrevista finalizada"`, consistente con el resto del criterio de "el candidato nunca ve evaluación".
  - [ ] P.11.2 Confirmar visualmente en el navegador que el título nuevo aparece al finalizar una entrevista.
