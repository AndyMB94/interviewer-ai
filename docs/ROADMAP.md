...
- [ ] 4.1 `DEBUG` controlado por variable de entorno (`os.environ.get("DEBUG", "False") == "True"` o similar) — `False` en el `.env` de producción, `True` en local/dev. Revisar qué otras cosas de `settings.py` asumían `DEBUG=True` implícitamente (ej. el `if settings.DEBUG:` en `config/urls.py` que sirve `MEDIA_URL` — con `DEBUG=False` en producción eso deja de aplicar, pero `/media/` ya lo sirve Nginx directo vía proxy a Django igual, hay que confirmar que no dependía de esa rama).
- [ ] 4.2 `STATIC_ROOT` en `settings.py` (ej. `BASE_DIR / "staticfiles"`) + `python manage.py collectstatic` corrido como parte del build/arranque del contenedor — junta los estáticos de todas las apps (incluyendo el admin) en una sola carpeta.
- [ ] 4.3 Nginx: `location /static/` deja de proxyear a Django (`proxy_pass`) y pasa a servir la carpeta de `collectstatic` directo con `alias` — necesita que esa carpeta sea accesible desde el host (volumen), ya que Nginx corre fuera de Docker.
- [ ] 4.4 Reemplazar `CMD ["python", "manage.py", "runserver", "0.0.0.0:8000"]` en `backend/Dockerfile` por `gunicorn` (agregar a `requirements.txt`) — el servidor real para producción, no el de desarrollo de Django.
- [ ] 4.5 Verificación completa: admin con estilos correctos en ambos dominios, un error 500 forzado a propósito ya no expone stack trace/config (con `DEBUG=False`, Django muestra una página de error genérica), toda la app (postular, entrevista, dashboard) sigue funcionando igual que antes del cambio de servidor.
