...
- [ ] P.9 Logo de la navbar consciente del rol (agregado 2026-08-13, encontrado al revisar si un Postulante logueado puede volver a `/entrevista` después de navegar afuera). Hoy `Navbar.tsx` linkea el logo "Vacantia" siempre a `/` (la grilla pública) sin importar el rol — dos problemas reales, no cosméticos:
  - Un **Postulante** que navega afuera de `/entrevista` (antes de empezar, o después de terminar — el bloqueo de P.8.1/Frontend 7.3 solo cubre mientras la entrevista está activa sin terminar) no tiene forma de volver salvo escribiendo la URL a mano; el dropdown de cuenta solo tiene el email y "Cerrar sesión".
  - Un **Reclutador** que hace click en el logo sale de su panel hacia la grilla pública de postulantes — no tiene sentido para alguien logueado con ese rol, debería volver a `/dashboard`.
  - [ ] P.9.1 `Navbar.tsx` — el logo linkea según el rol: sin sesión → `/`, `roles.includes("Postulante")` → `/entrevista`, `roles.includes("Reclutador")` → `/dashboard`. Usa `roles` que `AuthContext` ya expone (mismo criterio que el redirect post-login en `LoginPage.tsx`).
  - [ ] P.9.2 Probado end-to-end: logo lleva a cada destino correcto según con qué cuenta se esté logueado, y sigue yendo a `/` sin sesión.
- [ ] P.10 Composer unificado para la entrevista (agregado 2026-08-13, tras revisar el panel de chat del candidato). Hoy `TextAnswerForm.tsx` y `VoiceRecorder.tsx` son dos `Card` separadas apiladas debajo del chat, cada una con su propio título ("Responder por texto" / "Responder por voz") — se ve como dos formularios pegados, no como un chat real. Los chats reales (WhatsApp, ChatGPT, Messenger) tienen un solo "composer": input de texto + ícono de mic + botón de enviar, todos en la misma fila, sin títulos de sección.
  - **Componente nuevo `MessageComposer.tsx`**, reemplaza a `TextAnswerForm.tsx` y `VoiceRecorder.tsx` (se borran). Usa `InputGroup`/`InputGroupInput`/`InputGroupAddon` (`align="inline-end"`)/`InputGroupButton` de shadcn (instalar con `npx shadcn@latest add input-group`, no está instalado todavía) — sigue la regla del framework "botones dentro de un input van en `InputGroup`+`InputGroupAddon`", en vez de un `<div className="flex gap-2">` con `Input`+`Button` sueltos como hoy.
  - **Un solo botón de mic con 3 estados visuales** (ícono `Mic` de lucide, cambia de ícono/color según el estado — no hace falta un componente nuevo por estado):
    1. Sin permiso todavía: ícono `Mic` normal: al hacer click llama a `requestPermission()` (comportamiento existente de `useMicrophone`, sin cambios).
    2. Permiso concedido, sin grabar: mismo ícono `Mic`, ahora el click arranca a grabar directo (`startRecording()`) — un click menos que hoy, que exigía ver "Micrófono habilitado" y después apretar un botón "Grabar" aparte.
    3. Grabando: ícono cambia a `Square` (parar), `variant="destructive"`, con el punto pulsante "Grabando..." ya existente (`animate-pulse`) al lado, no en una línea separada.
  - **Errores de micrófono** (permiso denegado, etc.) se muestran como texto chico debajo del composer (`text-destructive`, ícono `CircleAlert` — reusa el patrón visual que ya tenía `VoiceRecorder`), no en una card aparte.
  - Botón de enviar (texto) sigue mandando con Enter o click, sin cambios de comportamiento — solo cambia dónde vive visualmente (adentro del `InputGroupAddon`, no un `Button` suelto al lado del `Input`).
  - `InterviewPage.tsx` — reemplaza el bloque `<TextAnswerForm />` + `<VoiceRecorder />` por `<MessageComposer />`, pasándole las mismas props que ya recibían ambos (nada cambia de `useMicrophone`/`useSocket`, es puramente de presentación).
  - Verificación manual en el navegador con micrófono real (no solo revisión de código, ya que sí hay un cambio de comportamiento: un click menos para empezar a grabar) — confirmar que el flujo completo (permiso → grabar → parar → se manda el audio) sigue funcionando igual que antes, solo con menos fricción visual.
