...
- [ ] 9.11.1 Backend: `Puesto.vacantes` gana `validators=[MinValueValidator(1)]` — migración de esquema.
- [ ] 9.11.2 Backend: `PuestoViewSet.get_queryset()` — el listado público (sin `?mias=true`) filtra `estado=Puesto.Estado.ABIERTO`; con `?mias=true` (el reclutador viendo sus propios puestos) se siguen mostrando todos, abiertos y cerrados, para poder reabrirlos.
- [ ] 9.11.3 Backend: `PostulacionViewSet.perform_create` — rechaza con 400 si `puesto.estado != Puesto.Estado.ABIERTO` ("Este puesto ya no acepta postulaciones."), defensa real en el backend, no solo ocultar el botón en el frontend.
- [ ] 9.11.4 Frontend: `PuestoFormPage.tsx` (crear/editar, ver Decisión 1) — campos `titulo`, `descripcion`, `funciones`, `requisitos`, `requisitos_deseables`, `modalidad` (`Select`), `vacantes` (number, min 1), `categoria` (`Select`, reusa `fetchCategorias` ya existente de 9.9). Rutas nuevas en `router.tsx`: `/dashboard/puestos/nuevo`, `/dashboard/puestos/:id/editar`, protegidas igual que el resto de `/dashboard` (`RequireRole` Reclutador).
- [ ] 9.11.5 Frontend: `PuestosPage.tsx` — botón "Nuevo puesto" (linkea a 9.11.4), botón "Editar" y botón "Cerrar puesto"/"Reabrir puesto" por fila (con `AlertDialog` solo para cerrar, ver Decisión 3 — reabrir no tiene la misma consecuencia, no hace falta confirmar).
- [ ] 9.11.6 Frontend: `PuestoDetailPage.tsx` — si `puesto.estado === "cerrado"`, oculta el formulario de postulación y muestra un aviso ("Este puesto ya no está aceptando postulaciones") en vez de dejar que el candidato lo intente y reciba el 400 de 9.11.3.
- [ ] 9.11.7 Tests backend: crear/editar/cerrar solo por el reclutador dueño (403 para otro reclutador, 401 sin autenticar); listado público excluye `cerrado`, `?mias=true` no; `Postulacion` a un puesto `cerrado` devuelve 400; `vacantes=0` rechazado por el modelo.
- [ ] 9.11.8 Verificación end-to-end en el navegador: crear un puesto nuevo desde el dashboard, verlo aparecer en `/` (público), editarlo, cerrarlo, confirmar que desaparece de `/` y que `PuestoDetailPage` ya no deja postular, reabrirlo y confirmar que vuelve a aparecer.
...
- [ ] 8.5.1 Backend: `ApplicantProfileSerializer` (`ModelSerializer`, todos los campos de `ApplicantProfile` menos `user`, editables).
- [ ] 8.5.2 Backend: `PerfilView(generics.RetrieveUpdateAPIView)`, `permission_classes=[IsPostulante]`, `get_object` devuelve el perfil del usuario autenticado (`get_or_create` por si acaso, aunque en el flujo normal 9.4 ya lo crea). Ruta `GET/PATCH /api/auth/perfil/`.
- [ ] 8.5.3 Frontend: `PerfilPage.tsx`, ruta `/perfil` (protegida, `RequireRole` Postulante). Formulario con los campos de `ApplicantProfile` — `departamento`/`provincia`/`distrito` como 3 `Select` en cascada (cada uno se habilita/repuebla cuando el anterior cambia, consumiendo los 3 endpoints de ubigeo ya existentes desde 8.4, sin llamar a la API externa directo).
- [ ] 8.5.4 Frontend: entrada "Mi perfil" en el `DropdownMenu` de cuenta de `Navbar.tsx`, visible solo para el rol Postulante (mismo criterio que el resto de la navegación condicionada por rol).
- [ ] 8.5.5 Tests backend: `GET`/`PATCH` del propio perfil funciona; un Reclutador no tiene acceso (403, `IsPostulante`); actualizar con un `numero_documento` duplicado (mismo `tipo_documento`) respeta el `UniqueConstraint` ya existente (400, no 500).
...
- [ ] 6.6.1 Evaluar e instalar una librería de datos/grilla de emojis para React (candidatas: `emoji-picker-react`, `emoji-mart`) — confirmar en ese momento que sea compatible con React 19 (el proyecto ya está en React 19, y no todas las librerías de este tipo se actualizan rápido a versiones nuevas de React).
- [ ] 6.6.2 `EmojiPickerButton.tsx` — botón con ícono (`Smile` de `lucide-react`, ya usado en el resto del proyecto) que abre un `Popover` de shadcn con la grilla de la librería elegida.
- [ ] 6.6.3 Lógica de inserción en la posición del cursor (Decisión 2), como un hook o helper reusado por las 5 instancias del botón en `PuestoFormPage.tsx`.
