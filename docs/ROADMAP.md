...
- [ ] P.7 Silent refresh de sesión al cargar la app (agregado 2026-08-12 — costo conocido desde el 2026-08-11, ver DECISIONS.md: el access token vive solo en memoria, se pierde al refrescar la página aunque la cookie del refresh token siga viva 7 días; se sintió más al usar login/dashboard de verdad en Frontend Fase 6).
  - [ ] P.7.1 Backend: `CustomTokenObtainPairSerializer` (`apps/accounts/serializers.py`) agrega también `email` como claim del JWT (además de `groups`, agregado en 6.1) — así el refresh silencioso puede reconstruir la sesión completa sin depender de lo que el usuario tipeó en el form de login.
  - [ ] P.7.2 Frontend: `lib/api.ts` — nueva función que llama a `POST /api/auth/token/refresh/` con `credentials: "include"` (usa la cookie httpOnly, sin pedir credenciales).
  - [ ] P.7.3 Frontend: `AuthContext` intenta ese refresh silencioso al montar la app (una vez); si la cookie sigue siendo válida, restaura `accessToken`/`userEmail`/`roles` decodificando el JWT nuevo; si no, se queda como no logueado (sin mostrar error, es el caso normal de "nunca inició sesión" o "la cookie ya venció"). Se agrega un estado `isCheckingSession` para que `RequireAuth`/`RequireRole` esperen ese chequeo inicial antes de decidir redirigir a `/login` — si no, redirigirían de una antes de que el refresh silencioso tenga chance de completarse.
...
- [ ] 6.1 Login de reclutador — reusa `LoginPage`/`AuthContext` que ya existen (mismo endpoint `/api/auth/login/`, no se duplica el mecanismo). **Decidido**: el rol viaja como claim custom en el JWT (no un endpoint `/api/auth/me/` nuevo) — ver DECISIONS.md 2026-08-12. Backend: `CookieTokenObtainPairView` usa un `serializer_class` custom que agrega `groups` al payload vía `get_token()`. Frontend: `AuthContext` decodifica el JWT al loguear, `LoginPage` redirige a `/dashboard` (Reclutador) o `/entrevista` (Postulante); nueva ruta `/dashboard` protegida con un guard que exige el rol (extiende `RequireAuth`, no lo duplica).
