...
- [ ] P.6 Más pulido visual (agregado 2026-08-11, tras revisar qué faltaba después de P.5). Alcance:
  - [ ] P.6.1 Toggle de tema claro/oscuro manual en la navbar (ícono sol/luna). Hoy la app sigue `prefers-color-scheme` del sistema automáticamente (no es un bug, es intencional) — esto agrega la opción de que el usuario lo *anule* dentro de la app. Guardar la elección en `localStorage`, aplicar/quitar la clase `.dark` en `<html>` (el CSS ya tiene ese bloque preparado en `index.css`). Sin elección guardada, sigue respetando el sistema como hasta ahora (default).
