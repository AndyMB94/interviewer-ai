...
> Estado: **El círculo de entrevista por voz está completo y desplegado — [interviewer.andymallcco.dev](https://interviewer.andymallcco.dev)**. Hablás por el micrófono en el navegador → Frontend (React) → Gateway (Node/Socket.io) → Backend (Django/Celery) → Deepgram (STT) → DeepSeek (LLM, con el historial de la entrevista) → ElevenLabs (TTS) → la respuesta se reproduce sola, con voz, en el navegador. Todo vía Redis pub/sub (sin polling), dockerizado detrás de Nginx + HTTPS en un VPS real. **En construcción:** el proyecto está pivotando de herramienta de práctica a una plataforma de reclutamiento con IA (nombre nuevo: **Vacantia**, pendiente de aplicar) — puesto → postulación con CV → filtro con IA → cuenta automática → login → entrevista. El embudo completo ya funciona de punta a punta (confirmado en local): postular sin cuenta con CV real → DeepSeek evalúa el fit → si aprueba, se crea la cuenta y se manda la contraseña por email (Resend) → login (JWT en memoria + cookie httpOnly) → entrevista protegida, con el JWT viajando hasta Django a través del gateway. El backend (Fases 8-9) ya está desplegado en producción; el frontend/gateway de este flujo (postulación, login, entrevista protegida) todavía no. Falta el panel de reclutador. Ver [docs/ROADMAP.md](docs/ROADMAP.md) para el detalle completo.
